...
Read this file, then `RESEARCH_IMPLEMENTATION_GAP_ANALYSIS.md` (source of truth for gaps/specs) and `BUILD_LOG.md` (evidence chain through Phase 61 plus hotfixes). CLAUDE.md's reference to the `...IDE__.md` research doc is stale — that content lives inside `OpenRouter First Blueprint...FULL RESEARCH.md`.
...
2. **Installed build/product proof:** Antigravity lists `kennyg.forge-agent@0.64.0`; local extension-host evidence proves network-intent governance, enforced role capability/session boundaries, external workspace-tool processes, blocker lifecycle, and mocked hybrid semantic retrieval mechanics. The prior 0.57.5 structured state remains historical live-model evidence; no credential-backed 0.64.0 semantic/product run is claimed.
...
6. Tier-3 slice 2: REAL repos in Phase-35 worktrees. Then: chunked explorers, interlock signature caps, plan-verification delegation, structured failure taxonomy, V4A patch format, skill registry (procedures, not just lessons), separate/worktree-backed parallel workers, socket-level network containment, and OS sandboxing BEFORE unattended real-repo runs.
