...
2. **Installed build/product proof:** Antigravity lists `kennyg.forge-agent@0.58.1`; installed compiled markers confirm immutable eval archives and the context scheduler/fair-share allocator. The prior `.tmp/antigravity-live-goal-0575-git/.forge/state.json` remains a valid structured success artifact, but `artifacts/antigravity-live-goal-0575-git-final.png` currently shows Antigravity onboarding rather than the Forge terminal state, so fresh installed-webview visual proof is still required.
3. Repeat live Tier-4 runs for means and task-level variance; Phase 58 will preserve each run instead of letting later mocks overwrite it. Current shell has no `OPENROUTER_API_KEY`.
