...
const transactionalCommands = read('src/harness/transactionalCommands.ts');
const proceduralSkills = read('src/harness/proceduralSkills.ts');
const workflowGovernance = read('src/harness/workflowGovernance.ts');
assert.match(transactionalEdits, /worktree', 'add', '--detach'/, 'git-backed edits must execute in detached worktrees');
assert.match(transactionalEdits, /sourceHashAtMerge !== sourceHashBefore/, 'transaction merge must reject concurrent source changes');
assert.match(transactionalEdits, /sparse-copy/, 'non-git edit transactions must use an explicit sparse staging fallback');
assert.match(transactionalEdits, /worktree', 'remove', '--force'/, 'temporary edit worktrees must be removed deterministically');
assert.match(transactionalCommands, /worktree', 'add', '--detach'/, 'git-backed commands must execute in detached worktrees');
assert.match(transactionalCommands, /sourceBefore\.files\.get\(rel\)/, 'command merge must compare every changed source path');
assert.match(transactionalCommands, /MAX_CHANGED_FILES/, 'command transactions must cap broad filesystem mutation');
assert.match(transactionalCommands, /rollbackAttempted: true/, 'partial command merge failures must trigger rollback');
assert.match(transactionalCommands, /unsupported symbolic links/, 'command transactions must reject symlink escape surfaces');
assert.match(transactionalCommands, /referencesSourceRoot/, 'command transactions must reject explicit active-workspace paths');
assert.match(transactionalCommands, /worktree', 'remove', '--force'/, 'temporary command worktrees must be removed deterministically');
assert.match(firewall, /'git commit'/, 'command policy must block shared git history mutation');
assert.match(firewall, /'git config'/, 'command policy must block shared git configuration mutation');
assert.match(proceduralSkills, /terminalStatus !== 'success'/, 'failed runs must never bank procedural skills');
assert.match(proceduralSkills, /verified-runs=/, 'skill prompt provenance must expose verified successful-run count');
assert.match(proceduralSkills, /appliedSessionIds/, 'skill usage must deduplicate repeated prompt assembly in one session');
assert.match(proceduralSkills, /run_tests pass and evidence ledger proof|Run the verification oracle|Rerun the same oracle/, 'banked workflows must retain deterministic verification steps');
assert.match(workflowGovernance, /CLASSIFY|classify/, 'workflow governance must classify task lanes');
assert.match(workflowGovernance, /Implementation requires completed baseline, reconcile, and documented-plan stages/, 'workflow must block early implementation');
assert.match(workflowGovernance, /Success declaration requires completed workflow stages/, 'workflow must block premature success');
assert.match(workflowGovernance, /workflow-task-record\.md/, 'workflow acceptance contract must name its durable task record');
assert.match(workflowGovernance, /terminalStatus !== 'success'|finalStatus = 'VERIFIED'/, 'workflow completion must remain tied to terminal verified state');
...
assert.match(loop, /TransactionalEditExecutor/, 'file mutations must use isolated transactional staging');
assert.match(loop, /worker-edit-transactions\.json/, 'edit transaction ledger must persist');
assert.match(loop, /worker-command-transactions\.json/, 'command transaction ledger must persist');
assert.match(loop, /selectProceduralSkills/, 'main product prompt must retrieve procedural skills');
assert.match(loop, /bankProceduralSkills/, 'terminal AAR must bank successful recovery procedures');
assert.match(loop, /validateWorkflowProposal/, 'main proposal validation must enforce workflow order');
assert.match(loop, /workflow-governance\.json/, 'workflow governance ledger must persist');
assert.match(loop, /workflow-task-record\.md/, 'workflow task record must persist');
